...
public class Swimmer extends Thread {
   ...
   public SwimTeam strokeTeam;
	...
	//Constructor
	Swimmer( int ID, int t, PeopleLocation loc, FinishCounter f, int speed, SwimStroke s, SwimTeam strokeTeam) {
		this.swimStroke = s;
		this.ID=ID;
	...
	public void run() {
		try {
         ...
         // Initialize a CountDownLatch for 10 swimmers, ensuring the first 10 threads wait before diving
         stop = new CountDownLatch(10);
         
         
			sleep(movingSpeed+(rand.nextInt(10))); //arriving takes a while
			myLocation.setArrived();

         // Depending on the swimmer's stroke order, wait for the previous swimmer(s) to finish their part of the race
         if (swimStroke.order==2) {strokeTeam.latch.await();}
         if (swimStroke.order==3) {strokeTeam.firstlatch.await();}
         if (swimStroke.order==4) {strokeTeam.secondlatch.await();}
         
         enterStadium();	
         
         // Notify that the swimmer has completed their portion and the next swimmer can now prepare
         if (swimStroke.order==1) {strokeTeam.latch.countDown();}
         if (swimStroke.order==2) {strokeTeam.firstlatch.countDown();}
         if (swimStroke.order==3) {strokeTeam.secondlatch.countDown();}
                
         
         goToStartingBlocks();
         
         // Wait for all swimmers (threads) to reach the starting blocks before proceeding
         stop.countDown();
         stop.await();
         
         // Synchronized block to ensure only one swimmer dives and swims at a time
         synchronized(strokeTeam) {      
                  dive();
                  swimRace();
         }
         
         
			if(swimStroke.order==4) { // if last swimmer finish line
